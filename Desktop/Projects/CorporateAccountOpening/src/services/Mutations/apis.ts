...
import { d,e } from "../../utils/utilities";
...
export const sendSMSEnc = async (data: string) => {
  var config = {
    method: "POST" as Method,
    url: "https://openapi.suntrustng.com/SendSmsEncrypted/api/Sms/SendSmsEncrypt",
    headers: {
  ...
  axios(config)
    .then((response) => {
      ...
      const dekrpt = d(response?.data?.data);

      // console.log(JSON.parse(dekrpt));
    })
    .catch((error) => {
...
export const sendEmailEnc = async (data: string) => {
  var config = {
    method: "POST" as Method,
    url: "https://openapi.suntrustng.com/SendSmsEncrypted/api/Sms/SendEmailEncrypt",
    headers: {
      "Content-Type": "application/json",
      // 'Authorization': `Bearer ${token}`
    },
